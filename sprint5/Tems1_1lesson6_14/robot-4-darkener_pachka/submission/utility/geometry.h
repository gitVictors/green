...
struct CoordinateF {
    ...
    bool operator==(const CoordinateF& other) const {
        double epsilon = std::numeric_limits<double>::epsilon();

        return std::abs(x_pos - other.x_pos) < epsilon && std::abs(y_pos - other.y_pos) < epsilon &&
               std::abs(z_pos - other.z_pos) < epsilon;
    }
