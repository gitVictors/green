...
public:
    FloorTile( const GameContext& context, std::string assert_name)
        : Tile(context.asset_loader.LoadTile("floors", assert_name))
    {

    }

    void Draw(DrawContext& context, Coordinate pos) const override{
...
public:
    ...
    void Draw(DrawContext& context, Coordinate pos, Orientation dir) const override {
        if(!is_opened_){
            ...
            context.painter.DrawRect(p4, p5, p6, door_color, edge_color, edge_width);
        }
    }
