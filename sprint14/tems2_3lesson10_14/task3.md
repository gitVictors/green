Задание 3
Реализуйте метод Emplace в классе Optional. Этот метод должен создавать значение внутри Optional путём прямой передачи параметров его конструктору. Если в Optional содержалось значение, оно должно быть предварительно разрушено перед созданием нового значения.
Сохраните решение на GitHub — оно вам ещё понадобится.
Для удобства используйте набор тестов:
tests.cpp
...
Тренажёр проверит работу метода Emplace, используя тесты подобные тем, которые даны в условии задачи. Функция main будет заменена версией из тренажёра.