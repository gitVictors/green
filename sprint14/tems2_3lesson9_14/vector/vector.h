...
public:
    ...
    //         // Используем Swap для безопасного перемещения
...
private:
    ...
    void EmplaceBack(Arg&& arg) {
        if (size_ == Capacity()) {
            EmplaceBackWithRealloc(std::forward<Arg>(arg));
        } else {
    ...
    template <typename Arg>
    void EmplaceBackWithRealloc(Arg&& arg) {
        const size_t new_capacity = size_ == 0 ? 1 : size_ * 2;
        RawMemory<T> new_data(new_capacity);

        // Сначала создаем новый элемент (если бросит исключение - старые данные не тронуты)
        new (new_data + size_) T(std::forward<Arg>(arg));

        // Затем перемещаем/копируем существующие элементы
        try {
            if constexpr (std::is_nothrow_move_constructible_v<T> || !std::is_copy_constructible_v<T>) {
                std::uninitialized_move_n(data_.GetAddress(), size_, new_data.GetAddress());
            } else {
            ...
            // Если перемещение/копирование не удалось - уничтожаем новый элемент
            std::destroy_at(new_data + size_);
            throw;
        }

        // Уничтожаем старые элементы и заменяем буфер
        std::destroy_n(data_.GetAddress(), size_);
        data_.Swap(new_data);
        ++size_;
    }
