...
public:
    ...
    int64_t GetCurrentBill(const VehiclePlate& car) const {
        ...
        if (now_parked_.count(car)) {
            auto duration = Clock::now() - now_parked_.at(car);
            result += chrono::duration_cast<chrono::seconds>(duration).count() * cost_per_second_;
        }
    ...
    //     for (auto& [car, dur] : complete_parks_){
    //         int64_t cost = chrono::duration_cast<chrono::seconds>(dur).count() * cost_per_second_;
    //         if (cost > 0)
    //             result[car] = cost;
    //     }
    ...
    //         int64_t cost = chrono::duration_cast<chrono::seconds>(dur).count() * cost_per_second_;
    //         if (cost > 0) {
    //             result[car] += cost;  //chrono::duration_cast<chrono::seconds>(dur).count() * cost_per_second_;
    //         }
    //         when = now;
    //     }
    //     return result;
    ...
    unordered_map<VehiclePlate, int64_t, VehiclePlateHasher> EndPeriodAndGetBills() {
        ...
        // Обрабатываем завершенные парковки
        for (auto& [car, dur] : complete_parks_) {
            int64_t cost = chrono::duration_cast<chrono::seconds>(dur).count() * cost_per_second_;
            if (cost > 0) {
                result[car] = cost;
        ...
        // Обрабатываем текущие парковки и сбрасываем их время
        auto now = Clock::now();
        for (auto& [car, when] : now_parked_) {
            auto dur = now - when;
            int64_t cost = chrono::duration_cast<chrono::seconds>(dur).count() * cost_per_second_;
            ...
            when = now;  // Сбрасываем время начала парковки
        }
