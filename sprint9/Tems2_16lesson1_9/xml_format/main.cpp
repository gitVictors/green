...
vector<Spending> LoadFromXml(istream& input) {
    ...
    const Node& root = dc.GetRoot();

    for (const Node& spend : root.Children()){
        val.amount = spend.AttributeValue<int>("amount");
        val.category = spend.AttributeValue<string>("category");
